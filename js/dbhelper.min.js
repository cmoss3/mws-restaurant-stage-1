class DBHelper{static get DATABASE_URL(){const port=1337;return `http://localhost:${port}/restaurants`}
    static fetchRestaurants(callback){const database=new browserDB();database.access(store=>{const request=store.get(DBHelper.DATABASE_URL);request.onsuccess=()=>{if(typeof request.result!=='undefined'){callback(null,request.result)}else{fetch(DBHelper.DATABASE_URL).then(response=>response.json()).then(data=>{callback(null,data);const database=new browserDB();database.access(store=>store.put(data,DBHelper.DATABASE_URL))}).catch(e=>{console.log(`data fetch failure: ${e}`);callback(e,null)})}}})}
